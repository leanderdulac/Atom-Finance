...
echo ""
echo "  6. (Optional) Set up TLS: set ATOM_DOMAIN in ${PROD_ENV} and point DNS at"
echo "     this server, then bring the stack up with the Caddy edge compose file"
echo "     instead of 'systemctl start atom' above (see docs/PILOT-DEPLOYMENT.md):"
echo "     cd ${APP_DIR} && docker compose -p atom --env-file ${PROD_ENV} \\"
echo "       -f docker-compose.prod.yml -f docker-compose.edge.yml up -d --build --wait"
echo "     Future updates then go through 'ATOM_ENABLE_EDGE=true scripts/deploy.sh',"
echo "     not systemctl, so the edge compose file stays included."
echo ""
