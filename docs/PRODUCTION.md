...
4. Aplicar as migrations contra o banco do passo 1 antes do primeiro deploy: `cd backend && ATOM_DATABASE_URL=... alembic upgrade head` (rodar de uma máquina com acesso à rede do banco, ou via `docker compose exec backend alembic upgrade head` após o primeiro `up`).
...
8. Manter `ATOM_ALLOW_REGISTRATION=false`. Limites de API usam o endereço visto pelo backend; nesta configuração conservadora, clientes atrás do proxy compartilham a cota. Configurar confiança de proxy e limites por identidade antes de ampliar o público.
...
- Health HTTP não substitui monitoramento: configurar alerta externo de 503, reinícios, disco, idade do backup e erros dos provedores. Redis é cache descartável; a persistência principal é o Postgres gerenciado.
