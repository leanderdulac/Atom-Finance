...
- Backend sem root; segredos, bancos e ambientes virtuais excluídos dos contextos Docker. Frontend publicado somente em loopback por padrão. Limite HTTP de 2 MB no nginx. Content-Security-Policy restringe a origens próprias (mais Google Fonts); `frame-ancestors 'none'` complementa o `X-Frame-Options: DENY` contra clickjacking.
- Cada requisição recebe um `X-Request-ID` (aceita o do proxy/cliente se enviado, gera um se não) e toda linha de log da aplicação durante aquela requisição carrega o mesmo id — inclusive a de um erro de readiness do banco — o que permite isolar os logs de uma requisição específica num servidor com tráfego concorrente. Rastreamento de erro (Sentry) é opcional: fica completamente desligado até `SENTRY_DSN` ser definido, sem nenhuma outra dependência no restante do app. Métricas Prometheus (`ATOM_ENABLE_METRICS=1`) expõem `GET /metrics` **na porta do backend, fora de `/api`**. O nginx do frontend só faz proxy de `/api/`; o backend em prod usa `expose: 8000` na rede `internal`, sem publicar a porta no host. O scraper precisa estar no mesmo Docker network — nunca abrir `8000` na internet.
...
- Health HTTP não substitui monitoramento: configurar alerta externo de 503, reinícios, disco, idade do backup e erros dos provedores. Redis é cache descartável; a persistência principal é o Postgres gerenciado. Definir `SENTRY_DSN` cobre a parte de captura de exceção não tratada. Com `ATOM_ENABLE_METRICS=1` o Prometheus raspa `backend:8000/metrics` na rede interna (não o bind 8080). Tracing de performance continua desligado (`SENTRY_TRACES_SAMPLE_RATE` 0.0 por padrão).
